...
async function deleteInactiveUserData(m) {
  ...
  if (!user) return; // إذا كان المستخدم غير موجود، فإنه لا يفعل شيئا
  ...
  if (currentTime - lastUpdateTime > INACTIVITY_TIMEOUT_MS) {
    ...
    //console.log(`بيانات المستخدم ${m.sender} حذفت بعد ${INACTIVITY_TIMEOUT_MS / 1000 / 60} دقائق من عدم النشاط.`);
  }
}
...
export async function all(m) {
  ...
  if (user) {
    ...
    return; // إذا كان المستخدم غير موجود، فإنه لا يفعل شيئا
  }
